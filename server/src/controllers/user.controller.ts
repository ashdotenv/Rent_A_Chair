...
export const updateProfile = catchAsyncError(async function (
  ...
  if (req.files?.profilePic) {
    const file = req.files.profilePic as Express.Multer.File;
    try {
...
export const getMyDetails = catchAsyncError(async function name(
  req: Request,
  res: Response,
  next: NextFunction
) {
...
export const placeOrder = catchAsyncError(async function (
  req: Request,
  res: Response,
  next: NextFunction
) {
    ...
    const order = await prisma.order.create({
      data: {
        ...
        totalPrice: orderData.reduce(
          ...
          0
        ), // Assuming totalPrice is in each orderData item
        orderItems: {
...
export const getMyOrders = catchAsyncError(async function (
  req: Request,
  res: Response,
  next: NextFunction
) {
